...
const eventsDataCopy = [
    ..._dataJs.theaterData.events
];
...
// Search events
_allEventsJs.searchHandler(_dataJs.theaterData.events);
// Filter events by date
_allEventsJs.btnFindHandler(_dataJs.theaterData.events);
// Upload and save new event
_allEventsJs.uploadBtnHandler(eventsDataCopy);
...
function addEventCalendar() {
    ...
    dayContainer.addEventListener('click', (e)=>{
        ...
        if (btn.classList.contains('calendar-28')) window.location.assign('event.html');
    });
}
...
},{"../helper.js":"gDUlg","@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}],"5xJk9":[function(require,module,exports) {
...
parcelHelpers.export(exports, "uploadBtnHandler", ()=>uploadBtnHandler
);
parcelHelpers.export(exports, "searchHandler", ()=>searchHandler
);
parcelHelpers.export(exports, "btnFindHandler", ()=>btnFindHandler
);
var _helperJs = require("./helper.js");
...
addHandlerHideForm();
const uploadBtnHandler = function(data) {
    if (!uploadBtn) return;
...
editEventHandler();
const searchHandler = function(events) {
    if (!events || !searchBtn || !eventsContainer) return;
    try {
        searchBtn.addEventListener('click', ()=>{
            const filteredEvents = events.filter((event)=>{
                if (searchInput.value === '' || searchInput.value === ' ') return;
                if (event.title.toLowerCase().includes(searchInput.value.toLowerCase())) return event;
            });
            eventsContainer.innerHTML = '';
...
const btnFindHandler = function(events) {
    if (!btnFind) return;
    try {
